...
function refreshIndexPage(state) {
    fetchTensorhubData("/data/runs", function(runs) {
...
function runLabel(run) {
    var label = "";
    if (run.start) {
        var start = new Date(run.start);
        label += start.toDateString() + ", " + start.toLocaleTimeString();
    }
    if (run.model) {
        if (label) {
            label += " - ";
...
function fetchTensorhubData(url, callback) {
    $.ajax({
...
function applyWidgetsDataSource(source, widgets, opAttr, state) {
    ...
    fetchTensorhubData(sourceUrl, handler);
}
